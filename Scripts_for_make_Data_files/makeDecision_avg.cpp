...
int main()
{
	...
	sprintf(outputFileName,"C:\\Users\\User\\Desktop\\피실험자%d\\OUTPUT_DATA\\output%02d_avg%02d_%.1f.txt",s,d,num,theta_c);
	FILE *input=fopen(inputFileName,"rb");
	FILE *output=fopen(outputFileName,"wb");
	
	data tempData;
	data Array[10][300]; //needs edit
	...
	for(int n=0;n<N;n++)
	{
		...
		for(int f=0;f<FPS*T;f++)
		{
			...
			if(f<num)
			{
				tempTheta[f]=theta;
				sumTheta+=theta;
				avgTheta=sumTheta/(f+1);
			}
			else
			{
				sumTheta-=tempTheta[f%num];
				sumTheta+=theta;
